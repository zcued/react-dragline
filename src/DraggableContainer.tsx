...
// lr === rl; tb === bt
type DirectionX = 'll' | 'rr' | 'lr' | 'mh'
type DirectionY = 'tt' | 'bb' | 'tb' | 'mv'
...
export class DraggableContainer extends React.Component<Props, State> {
  ...
  calcPosValues(values: Grid, target: DraggableElement, compares: Array<DraggableElement>, key: GridKey) {
    ...
    compares.forEach((compare) => {
      validDirections.forEach(direction => {
        const { near, dist, value, origin, length } = this.calcPosValuesSingle(values, direction, target, compare, key)
        if (near) {
  ...
  _renderChildren() {
    ...
    return React.Children.map<React.ReactElement, React.ReactElement>(children as React.ReactElement, (child, index) => {
